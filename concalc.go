...
func main() {
	...
	for {
		...
		//в алгоритме допускающим ввод без пробелов эта обработка не к месту
		/*
			if Str3 == "" {
		...
		*/
		...
		//что позволит использовать уже отработанный алгоритм

		for { //поиск арифметического знака и разбиение оставшегося текстого материала по этому знаку
			Str2 = "+"
			...
			if !OperandError {
				break // если не нашлость ни одного валидного аривметического знака выходим для начала из этого for;;
			}
		}
